...
/**
 * Metadata for tab destinations (fade transition when switching tabs).
 */
...
fun createAppEntryProvider(
    ...
    // ── Tab screens ──────────────────────────────────────────────────────────
    entry<DashboardKey>(metadata = TabTransitionMetadata) { _ ->
    ...
    entry<LocksKey>(metadata = TabTransitionMetadata) { _ ->
        Box(modifier = Modifier.fillMaxSize().padding(tabPadding)) {
            LocksScreen(prefs = prefs)
        }
    }

    entry<SecurityKey>(metadata = TabTransitionMetadata) { _ ->
        Box(modifier = Modifier.fillMaxSize().padding(tabPadding)) {
            SecurityScreen(prefs = prefs)
        }
    }

    entry<SettingsKey>(metadata = TabTransitionMetadata) { _ ->
        Box(modifier = Modifier.fillMaxSize().padding(tabPadding)) {
            SettingsScreen(
    ...
    entry<PermissionsKey>(metadata = FullScreenTransitionMetadata) { _ ->
        PermissionsScreen(
            onBackClick = { backStack.removeLastOrNull() }
        )
    }

    entry<AccountKey>(metadata = FullScreenTransitionMetadata) { _ ->
        AccountScreen(
            onBackClick = { backStack.removeLastOrNull() }
        )
    }

    entry<UsageBreakdownKey>(metadata = FullScreenTransitionMetadata) { _ ->
